...
def getLegs(data):
    global prev_legs
    leg = data.data
    if leg=="left" and prev_legs!='left':
        print "left"
        prev_legs = "left"
        l_step.execute()
    elif leg=="right" and prev_legs!='right':
        print "right"
        prev_legs = "right"
        r_step.execute()
...
if __name__=='__main__':
    ...
    # raw_input("Soch Le")
    # r_turn.execute()
    #boom_walk.execute(iter=10)
    rospy.init_node('shadow_bot', anonymous=True)
    #rospy.Subscriber('skel_hands', String, getHands)
    rospy.Subscriber('skel_legs',String,getLegs)
    rospy.spin()
